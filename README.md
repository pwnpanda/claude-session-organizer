...
- **`conversation-summary`** — at session end, a `SessionEnd` hook spawns a headless `claude --print --bare` child that reads the transcript and writes `.context-handoff.json` at the **git repo root** of the cwd (falling back to cwd if not in a repo). The summary is a structured `context-handoff-merged-v3` snapshot.
